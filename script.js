...
const appId = "5e394a1f70c15bfed309d68945fcc318" // API Key from https://home.openweathermap.org/api_keys  
...
const onPositionGathered = (pos) => {
	let lat = pos.coords.latitude.toFixed(4), lon = pos.coords.longitude.toFixed(4) //it gives coordinates.
...
const getAirQuality = async (lat, lon) => { //async makes a function return a Promise
	// Get data from api
	const rawData = await fetch(`${link}?lat=${lat}&lon=${lon}&appid=${appId}`).catch(err => { //await makes a function wait for a Promise
